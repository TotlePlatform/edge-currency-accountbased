...
// } from './ethTypes.js'
// import { XrpPlugin } from './ethPlugin.js'
...
export class EthereumEngine extends CurrencyEngine {
  ...
  async multicastServers (func: XrpFunction, params: string): Promise<any> {
    let out = { result: '' }
    switch (func) {
      ...
      case 'getTransactions':
        const etherscanApiServers = this.currencyPlugin.currencyInfo.defaultSettings.otherSettings.etherscanApiServers
        const funcs = etherscanApiServers.map(server => async () => {
          const result = await this.fetchGetEtherscan(server, params)
